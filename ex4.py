...
class BinarySearchTree:
    ...
    def _lr_rotate(self, pivot):
        son = pivot.left
        grandson = son.right
        self._left_rotate(son)
        if pivot is self.root:
            self.root = grandson
        else:
            ancestor = self.find_ancestor(pivot)
            if pivot.key <= ancestor.key:
...
def main():
    ...
    tree.insert(15) # Should return case 1
    tree.insert(14) # Should return case 3a
    tree.insert(13) # Should return case 3b
